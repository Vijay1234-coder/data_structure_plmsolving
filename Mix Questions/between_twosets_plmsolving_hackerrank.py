...
def getTotalX(a, b):
    ...
    lcm = reduce(LCM, a)
    gcd_var = reduce(gcd, b)

    lcm_increment_var = lcm

    count = 0
    while (lcm <= gcd_var):
        if (gcd_var % lcm) == 0:
            count += 1
        lcm += lcm_increment_var
...
if __name__ == '__main__':
    ...
    first_multiple_input = input("enter no of elemnt in 1st , 2nd array").rstrip().split()
    ...
    brr = list(map(int, input("enter elements").rstrip().split()))
