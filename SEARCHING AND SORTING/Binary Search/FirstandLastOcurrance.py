...
def firstOcurrance(arr,target):
    ...
    return "FirstOcurrance at index: {0}".format(firstResult)
...
def lastOcurrance(arr,target):
    ...
    while (start <= end):
        ...
        if arr[mid] == target:    # if mid is target then lastoccurance will b eon right side
            lastResult = mid
            start= mid + 1
    ...
    return "LastOcurrance at index: {0}".format(lastResult)


arr=[1,3,5,5,5,5,67,123,125]

print(firstOcurrance(arr,5),lastOcurrance(arr,5))
